...
def render_sidebar(datasets: dict) -> dict:
    ...
    warn_thresh  = st.sidebar.slider(
        "Warning threshold", 0.0, float(alarm_hi * 1.5), float(warn_lo), step=0.1,
        help="Amber alarm fires above this value",
    )
    alarm_thresh = st.sidebar.slider(
        "Alarm threshold", float(warn_thresh), float(alarm_hi * 2), float(alarm_hi),
...
def render_kpis(df: pd.DataFrame, target: str,
    ...
    col2.metric("95% PI width",        f"± {(hi - lo) / 2:.2f} mg/m³")
...
def render_timeseries(df: pd.DataFrame, target: str, window: int,
    ...
    fig.update_layout(
        **PLOTLY_LAYOUT,
        title=dict(
            text=f"{target} — Soft Sensor Prediction with 95% Uncertainty Interval",
...
def render_bottom_row(df: pd.DataFrame, target: str, end_idx: int):
    ...
    with col_right:
        ...
        fig_width.update_layout(
            **PLOTLY_LAYOUT,
            title=dict(
                text=f"{target} — 95% Prediction Interval Width over Time",
                font=dict(color=CLR["accent"], size=12),
            ),
...
def main():
    ...
    if cfg["live"]:
        ...
        while cfg["live"]:
            ...
            st.session_state.sim_idx = idx + 1 if idx < n - 1 else cfg["window"]
