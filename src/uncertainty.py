...
def finite_sample_quantile(scores: np.ndarray, alpha: float = ALPHA) -> float:
    """Conformal quantile with the finite-sample ``higher`` correction."""
...
def calibrate_mc_dropout(y_true: np.ndarray, y_pred: np.ndarray,
                         y_std: np.ndarray,
                         alpha: float = ALPHA) -> np.ndarray:
    """Calibrate MC-Dropout scale separately for each output.

    Normalized split-conformal scores retain varying MC-Dropout widths without
    assuming that 1.96 times the epistemic standard deviation is a calibrated
    ...
    if y_true.shape != y_pred.shape or y_true.shape != y_std.shape:
        raise ValueError("y_true, y_pred, and y_std must have identical shapes")
    if y_true.ndim != 2:
        raise ValueError("Expected arrays shaped (n_samples, n_outputs)")

    eps = np.maximum(np.median(y_std, axis=0) * 1e-3, 1e-8)
    scores = np.abs(y_true - y_pred) / np.maximum(y_std, eps)
    return np.array([
        finite_sample_quantile(scores[:, i], alpha) for i in range(scores.shape[1])
    ])
